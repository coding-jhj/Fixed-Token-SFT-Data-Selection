...
Primary IFEval prompt-level strict에서 diversity minus random 차이는 -0.26 percentage points였고, paired 95% bootstrap interval은 [-3.39, 2.60] percentage points였습니다. Primary hypothesis는 지지되지 않았습니다. Diversity는 secondary BBH subset에서 더 높았지만, 일반적 우위가 아닌 task-dependent evidence로 보고합니다. 출력 한도 진단은 3,984개 decoded row 중 3,937개가 설정된 generation limit에 도달했을 가능성을 표시했으므로, 더 긴 generation 평가가 후속 과제입니다.
...
- Selection policy: stratified random, quality-only, quality-plus-semantic-diversity; seed 13과 42.
- Evaluation subset: IFEval 192, GSM8K 256, BBH 216; selection seed 2026.
- Generation cap: IFEval 512 new tokens; GSM8K와 BBH 128 new tokens; greedy decoding; batch size 8.
